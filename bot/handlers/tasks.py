...
def register_tasks_handlers(dp: Dispatcher):
    ...
    async def list_tasks(event: MessageCreated):
        ...
        lines = [f"{t.id}. {'done' if t.status == 'done' else 'pending'} {t.text}" for t in tasks[:50]]
        text = "Задачи:\n" + "\n".join(lines)
        chunks = [text[i:i+3900] for i in range(0, len(text), 3900)]
        for i, chunk in enumerate(chunks):
            prefix = f"({i+1}/{len(chunks)}) " if len(chunks) > 1 else ""
            await event.message.answer(prefix + chunk)
    ...
    async def plain_text_task(event: MessageCreated):
        ...
        for i in range(1, 6):
            builder.row(CallbackButton(text=str(i), payload=str(i)))
    ...
    async def handle_callback(event: MessageCallback):
            data = event.callback.payload  # ← callback.payload!
